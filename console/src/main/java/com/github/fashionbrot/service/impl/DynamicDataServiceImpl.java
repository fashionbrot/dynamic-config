...
import com.github.fashionbrot.response.DataDynamicJsonResponse;
...
public class DynamicDataServiceImpl  extends ServiceImpl<DynamicDataMapper, DynamicDataEntity> implements DynamicDataService {
    ...
    final DynamicDataLogMapper dynamicDataLogMapper;
    final UserLoginService userLoginService;

    @Override
    public Object pageReq(DynamicDataRequest req) {
    ...
    @Override
    public Object checkVersion(DataDynamicApiRequest req) {
    ...
    @Override
    public ApiResponse getData(DataDynamicApiRequest req) {
        ...
        if ("1".equals(req.getAll())) {
            ...
            List<DataDynamicResponse> dynamicDataList = baseMapper.selectJsonByReq(map);
            if (CollectionUtils.isNotEmpty(dynamicDataList)){
                dynamicDataList = dynamicDataList
                        .stream()
                        .map(m-> DataDynamicResponse.builder()
            ...
            if (req.getVersion() != null && req.getVersion().longValue() == -1L) {
                return ApiResponse.builder().code(1).msg("应用未配置信息").build();
            }
            ...
            if (StringUtils.isEmpty(release.getTemplateKeys())) {
                return ApiResponse.builder().code(0).build();
            }
    ...
    private List<JSONObject> format(List<DataDynamicJsonResponse> jsonVos, String envCode, List<EnvVariableRelationEntity> variableRelationList){
    ...
    @Override
    public Long clusterSync(DataDynamicApiRequest req) {
        String key = getKey(req.getEnvCode(),req.getAppCode());
